# Installation


### Requirements

- PHP >= 8.2
- CakePHP 5.3+

### CakePHP Version Support

This version of the plugin supports CakePHP 5.3+ only.

### Installation via composer

First, require the package through Composer:

````
composer require orca-services/cakephp-swagger-ui
````

Then load the plugin in your application as documented in the section
[Loading a Plugin](https://book.cakephp.org/5.0/plugins.html#loading-a-plugin)
in the CakePHP CookBook.

### Installation alternatives

Refer to the CakePHP CookBook section
[Manually Installing a Plugin](https://book.cakephp.org/5.0/plugins.html#manually-installing-a-plugin).

---

Back to the [Documentation](../README.md).
